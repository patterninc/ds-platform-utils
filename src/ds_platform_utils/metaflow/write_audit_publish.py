...
if TYPE_CHECKING:
    from ds_platform_utils._snowflake.write_audit_publish import SQLOperation
...
def publish(  # noqa: PLR0913, D417
    ...
    query_comment_str = f"\n\n/* {json.dumps(tags)} */"
    original_query, annotated_query = annotate_sql_with_comment(query, query_comment_str)
    ...
    with conn.cursor() as cur:
        ...
        for operation in write_audit_publish(
            ...
            if hasattr(current, "card"):
                # Replace the annotated query with the original query for logging in the Metaflow card
                operation.query = original_query
...
def annotate_sql_with_comment(query: Union[str, Path], comment: str) -> Tuple[str, str]:
    """Append `comment` (e.g., /* {...} */) to every SQL statement in `query`.
    ...
    :param comment: Comment string to append to each SQL statement (e.g., "/* {...} */").

    :returns:
        A tuple of:

        - The original SQL query.
        - The modified SQL query with the comments added.
    ...
    # NOTE: We are returing both original and annotated query because in the publish function,
    # we want to log the original query in the Metaflow card.
    return query, annotated_query
